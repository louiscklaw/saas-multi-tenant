...
const ProductPricing = ({ plans, subscriptions }: ProductPricingProps) => {
  ...
  const initiateCheckout = async (price: string, quantity?: number) => {
    const res = await fetch(`/api/teams/${team?.slug}/payments/create-checkout-session`, {
      method: 'POST',
      headers: {
    ...
    if (data?.data?.url) {
      ...
      toast.error(data?.error?.message || data?.error?.raw?.message || t('stripe-checkout-fallback-error'));
    }
  };

  const hasActiveSubscription = (price: Price) => subscriptions.some((s) => s.priceId === price.id);
  ...
  return (
    <section className="py-3">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {plans.map((plan) => {
          return (
            <div className="relative rounded-md bg-white border border-gray-200" key={plan.id}>
              <div className="p-8">
                <div className="flex items-center space-x-2">
                  <h3 className="font-display text-2xl font-bold text-black">{plan.name}</h3>
                </div>
              ...
              <div className="flex justify-center flex-col gap-2 border-gray-200 px-8 h-10">
                {plan.prices.map((price: Price) =>
                  hasActiveSubscription(price) ? (
                    <Button key={price.id} variant="outline" size="md" fullWidth disabled className="rounded-full">
                      {t('current')}
                    </Button>
                  ) : (
                    <PaymentButton key={price.id} plan={plan} price={price} initiateCheckout={initiateCheckout} />
                  )
                )}
              </div>
